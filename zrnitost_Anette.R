...
median_values <- grain_box_new %>% 
  group_by(particlesize, aggregates, init_state, intensity) %>% 
  summarize(median_percentage = median(percentage, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(median_percentage = round(median_percentage, 1))

# Zobrazení výsledné tabulky
print(median_values)
